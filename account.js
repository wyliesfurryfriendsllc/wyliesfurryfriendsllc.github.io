...
function renderBookingsList(bookings) {
    ...
    filtered.forEach(b => {
        ...
        const emoji = firstPet.type === 'cat' ? PET_ICON_CAT : PET_ICON_DOG;
...
function renderPets() {
    ...
    userPets.forEach((pet, i) => {
        const emoji = pet.type === 'cat' ? PET_ICON_CAT : PET_ICON_DOG;
        const card  = document.createElement('div');
    ...
    bks.forEach(b => {
        ...
        const avatars = pets.slice(0,4).map((p, i) =>
            p.photoUrl
                ...
                : `<div class="cday-pet-avatar cday-pet-emoji" style="z-index:${4-i}">${p.type==='cat'?PET_ICON_CAT:PET_ICON_DOG}</div>`
        ).join('');
