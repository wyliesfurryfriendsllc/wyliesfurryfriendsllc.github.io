...
async function doForgotPassword() {
    ...
    try {
        // Google-only accounts have no password to reset; sendPasswordResetEmail
        // still resolves successfully in that case, so check providers first.
        try {
            const methods = await fetchSignInMethodsForEmail(auth, email);
            if (methods.length > 0 && !methods.includes('password')) {
                msg.textContent = "This email is linked to Google Sign-In. Please use the \"Continue with Google\" button instead — there's no password to reset.";
        ...
        msg.innerHTML = 'Reset email sent!<br>Check your inbox (and spam/junk folder).';
    ...
    panel.innerHTML = `
        ...
        <div class="detail-section">
            <button class="acct-copy-btn" onclick="openCopyBookingModal('${b.id}')">
                ...
                Copy this booking to New Dates
            </button>
        </div>` : ''}
        <div class="detail-section">
